...
function emitRoomState(roomId) {
    const snapshot = roomManager_1.roomManager.getSnapshot(roomId);
    if (!snapshot) {
        return;
    }
    /*
...
function requireMember(socket, rawRoomId) {
    const roomId = (0, validation_1.normalizeRoomId)(rawRoomId);
    if (!roomId) {
        sendError(socket, "Invalid room code.");
        return null;
    }
    if (socket.data.roomId !== roomId ||
        !roomManager_1.roomManager.getParticipant(roomId, socket.id)) {
...
app.post("/api/rooms", (req, res) => {
    const videoId = (0, validation_1.extractYouTubeId)(req.body?.videoId);
    if (!videoId) {
    ...
    const room = roomManager_1.roomManager.createRoom(videoId);
...
app.get("/api/rooms/:roomId", (req, res) => {
    const roomId = (0, validation_1.normalizeRoomId)(req.params.roomId);
    if (!roomId) {
        return res.status(400).json({
            success: false,
    ...
    const room = roomManager_1.roomManager.getSnapshot(roomId);
    if (!room) {
...
io.on("connection", (socket) => {
    ...
    socket.on("join_room", (payload) => {
        const roomId = (0, validation_1.normalizeRoomId)(payload?.roomId);
        const username = (0, validation_1.normalizeUsername)(payload?.username);
        if (!roomId || !username) {
            return sendError(socket, "Invalid room code or username.");
        }
        if (!roomManager_1.roomManager.getRoom(roomId)) {
        ...
        if (currentRoomId && currentRoomId !== roomId) {
            const previous = roomManager_1.roomManager.removeParticipant(currentRoomId, socket.id);
        ...
        const user = roomManager_1.roomManager.addParticipant(roomId, socket.id, username);
        if (!user) {
    ...
    socket.on("play", (payload) => {
        ...
        if (!roomManager_1.roomManager.hasPermission(roomId, socket.id, ["Host", "Moderator"])) {
            return sendError(socket, "Only Host or Moderator can play the video.");
        }
        const time = (0, validation_1.normalizeTime)(payload?.time);
        if (time === null) {
            return sendError(socket, "Invalid playback time.");
        }
        roomManager_1.roomManager.updatePlayback(roomId, "playing", time);
        emitRoomState(roomId);
    });
    /*
     * PAUSE
    ...
    socket.on("pause", (payload) => {
        ...
        roomManager_1.roomManager.updatePlayback(roomId, "paused", time);
        emitRoomState(roomId);
    });
    /*
     * SEEK
    ...
    socket.on("seek", (payload) => {
         ...
         */
        roomManager_1.roomManager.updatePlayback(roomId, room.playbackState, time);
        emitRoomState(roomId);
    });
    /*
     * CHANGE VIDEO
    ...
    socket.on("change_video", (payload) => {
        ...
        const videoId = (0, validation_1.extractYouTubeId)(payload?.videoId);
        if (!videoId) {
            return sendError(socket, "Invalid YouTube video URL or ID.");
        }
        roomManager_1.roomManager.updateVideo(roomId, videoId);
        emitRoomState(roomId);
    });
    /*
     * ROLE CHANGE
    ...
    socket.on("assign_role", (payload) => {
        ...
        if (!roomManager_1.roomManager.hasPermission(roomId, socket.id, ["Host"])) {
            return sendError(socket, "Only Host can assign roles.");
        }
        if (payload?.newRole !== "Moderator" &&
            payload?.newRole !== "Participant") {
            return sendError(socket, "Only Moderator or Participant roles can be assigned.");
        }
        if (payload.targetSocketId === socket.id) {
            return sendError(socket, "You cannot change your own Host role.");
        }
        const room = roomManager_1.roomManager.assignRole(roomId, payload.targetSocketId, payload.newRole);
        if (!room) {
            return sendError(socket, "Participant not found or role change is not allowed.");
        }
        /*
    ...
    socket.on("transfer_host", (payload) => {
        ...
        const room = roomManager_1.roomManager.transferHost(roomId, socket.id, payload.targetSocketId);
        if (!room) {
    ...
    socket.on("remove_participant", (payload) => {
        ...
        const target = roomManager_1.roomManager.getParticipant(roomId, payload.targetSocketId);
        if (!target) {
        ...
        const result = roomManager_1.roomManager.removeParticipant(roomId, payload.targetSocketId);
        if (!result) {
     ...
     * approve or reject.
     */
    socket.on("create_request", (payload) => {
        const roomId = requireMember(socket, payload?.roomId);
        if (!roomId) {
            return;
        }
        ...
        let videoId;
        if (type === "seek") {
            time = (0, validation_1.normalizeTime)(payload?.time) ?? undefined;
            if (time === undefined) {
                return sendError(socket, "Enter a valid seek time.");
            }
        }
        if (type === "change_video") {
            videoId = (0, validation_1.extractYouTubeId)(payload?.videoId) ?? undefined;
            if (!videoId) {
                return sendError(socket, "Enter a valid YouTube URL or video ID.");
            }
        }
        const participant = roomManager_1.roomManager.getParticipant(roomId, socket.id);
        if (!participant) {
            return;
        }
        const request = roomManager_1.roomManager.addRequest(roomId, socket.id, participant.username, type, time, videoId);
        if (!request) {
            return sendError(socket, "You already have a pending request. Wait for it to be handled first.");
        }
        emitRoomState(roomId);
    });
    socket.on("resolve_request", (payload) => {
        const roomId = requireMember(socket, payload?.roomId);
        if (!roomId) {
            return;
        }
        ...
        const request = roomManager_1.roomManager.getRequest(roomId, payload.requestId);
        if (!request) {
            return sendError(socket, "Request is no longer available.");
        }
        const requester = roomManager_1.roomManager.getParticipant(roomId, request.socketId);
        if (!requester) {
            roomManager_1.roomManager.removeRequest(roomId, request.id);
            emitRoomState(roomId);
        ...
        if (payload.approved) {
            switch (request.type) {
                case "play": {
                    const snapshot = roomManager_1.roomManager.getSnapshot(roomId);
                    roomManager_1.roomManager.updatePlayback(roomId, "playing", snapshot?.currentTime ?? 0);
                    break;
                }
                case "pause": {
                    const snapshot = roomManager_1.roomManager.getSnapshot(roomId);
                    roomManager_1.roomManager.updatePlayback(roomId, "paused", snapshot?.currentTime ?? 0);
                    break;
                }
                case "seek": {
                    const room = roomManager_1.roomManager.getRoom(roomId);
                    ...
                    roomManager_1.roomManager.updatePlayback(roomId, room.playbackState, request.time);
                    break;
                }
                case "change_video": {
                    if (!request.videoId) {
                    ...
                    roomManager_1.roomManager.updateVideo(roomId, request.videoId);
                    break;
                }
            }
        }
    ...
    socket.on("send_chat", (payload) => {
        ...
        const text = (0, validation_1.normalizeChatMessage)(payload?.text);
        if (!text) {
        ...
        const message = roomManager_1.roomManager.addMessage(roomId, socket.id, participant.username, text);
        if (!message) {
    ...
    socket.on("leave_room", (payload) => {
        ...
        const result = roomManager_1.roomManager.removeParticipant(roomId, socket.id);
        socket.leave(roomId);
        socket.data.roomId = null;
